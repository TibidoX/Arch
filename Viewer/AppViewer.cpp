...
void getInput(char* param, const char* name)
{
...
void runApp(std::string cmd) // run application, possibly with arguments. Example: runApp("AppServer.exe 12345");
{
    system(cmd.c_str());
}

void openImage(std::string file) // open image file with default viewer app
{
...
bool Viewer::init()
{
    ...
    char cmd[] = "SUBSCRIBE";
    int len = m_socket.send(cmd, sizeof(cmd)); // this is special request for the server that identifies subscription requesst
    ...
    while(1)
    {
        ...
        if(len > 0)
        {
            ...
            for (std::string str : m_data) {
                std::string path = "..\\Server" + join(split(str, "/"), "\\");
